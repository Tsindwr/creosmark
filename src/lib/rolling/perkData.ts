import type {
    PerkDefinition,
    PerkId,
    PerkResolutionContext, VolatilityDieResult,
    VolatilityDieType,
} from './types';
import {getMaxJinxThreshold} from "../volatility.ts";
...
export const BASE_PERKS: Record<PerkId, PerkDefinition> = {
    ...
    stabilize: {
        ...
        timing: "on-kept-die",
        description:
            "Treat your jinx threshold as 1 lower for this roll.",
    ...
    fracture: {
        ...
        resolve: (context) => ({
            face:
                context.keptFace <= context.jinxThreshold ? context.dieType : 1,
    ...
    tempt: {
        ...
        shortLabel: "Tempt",
        resolve: (context) => ({
            successModifier: context.keptFace === context.jinxThreshold + 1
                ? 2
                : undefined,
            notes:
